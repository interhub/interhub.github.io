...
(function () {
  ...
  function renderInfographic(t){
    ...
    clear(host);
    var g = t.infographic, c = g.cluster;
    var stepanSigma = parseFloat(g.sig) || 4.8;
    var NS = "http://www.w3.org/2000/svg";
    var CO = { paper:"#f2e9db", paperSoft:"#b7a793", hair:"#4a382c", terra:"#c8552b", clay:"#e0a24e" };

    var block = el("div", { class: "ig-block" });
    var head = el("div", { class: "ig-head reveal" }, [
      el("div", { class: "ig-eyebrow" }, [g.eyebrow]),
      el("p", { class: "ig-lead" }, [g.lead]),
    ]);
    var svg = document.createElementNS(NS, "svg");
    svg.setAttribute("class", "ig-chart");
    svg.setAttribute("preserveAspectRatio", "none");
    svg.setAttribute("role", "img");
    svg.setAttribute("aria-label", g.lead);
    block.appendChild(head);
    block.appendChild(svg);
    host.appendChild(block);

    function rng(seed){ var s = seed >>> 0; return function(){ s = (s * 1664525 + 1013904223) >>> 0; return s / 4294967296; }; }
    var dots = (function(){ var r = rng(20260715), out = [], tries = 0;
      while(out.length < 300 && tries < 200000){ tries++; var gg = 0; for(var i=0;i<3;i++) gg += r(); gg = (gg - 1.5) / 1.5 * 1.02;
        if(gg < -1.5 || gg > 4.5) continue; out.push({ s:gg, hf:r(), rad:0.7 + r()*0.9, op:0.16 + r()*0.5 }); }
      return out; })();
    function gaussf(x){ return Math.exp(-0.5 * x * x); }
    function E(tag, attrs){ var n = document.createElementNS(NS, tag); for(var k in attrs) n.setAttribute(k, attrs[k]); return n; }

    var geo = null, curGroup = null, curLine = null, curDot = null, curSig = null;
    function sx(s){ return geo.origin + s * geo.pxSig; }
    function sigAt(x){ return (x - geo.origin) / geo.pxSig; }

    function build(){
      var w = svg.clientWidth || block.clientWidth, h = svg.clientHeight || block.clientHeight;
      if(!w || !h) return;
      var padR = Math.max(20, w * 0.03);
      var origin = w * 0.115;
      var pxSig = (w - padR - origin) / 5.6;
      var botPad = 54, baseY = h - botPad;
      var padL = Math.max(16, w * 0.02);
      var headBottom = head.offsetTop + head.offsetHeight;
      var narrow = w <= 760;
      var peakY = narrow ? Math.min(headBottom + 34, baseY - 150) : Math.max(150, h * 0.34);
      geo = { w:w, h:h, padL:padL, origin:origin, pxSig:pxSig, baseY:baseY, peakY:peakY, ampl:baseY - peakY };
      var stepanNameY = narrow ? Math.max(headBottom + 6, 44) : Math.max(90, h * 0.13);

      svg.setAttribute("viewBox", "0 0 " + w + " " + h);
      while(svg.firstChild) svg.removeChild(svg.firstChild);
      var defs = E("defs", {});
      defs.innerHTML = '<filter id="igGlow" x="-120%" y="-120%" width="340%" height="340%"><feGaussianBlur stdDeviation="9" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>';
      svg.appendChild(defs);
      var fs = Math.max(10, Math.min(13, w * 0.0092));

      var frag = "";
      for(var i=0;i<dots.length;i++){ var d = dots[i]; var x = sx(d.s); if(x < padL || x > w - 6) continue;
        var top = baseY - geo.ampl * gaussf(d.s); var y = baseY - d.hf * (baseY - top);
        frag += '<circle cx="' + x.toFixed(1) + '" cy="' + y.toFixed(1) + '" r="' + d.rad.toFixed(2) + '" fill="' + CO.paper + '" opacity="' + d.op.toFixed(2) + '"/>'; }
      var dg = E("g", {}); dg.innerHTML = frag; svg.appendChild(dg);

      var pts = [], s = (padL - origin) / pxSig, end = Math.min(6, (w - 2 - origin) / pxSig);
      for(; s <= end; s += 0.04){ pts.push(sx(s).toFixed(1) + " " + (baseY - geo.ampl * gaussf(s)).toFixed(1)); }
      svg.appendChild(E("path", { d: "M " + pts.join(" L "), fill: "none", stroke: CO.paperSoft, "stroke-width": 1, opacity: 0.32 }));

      svg.appendChild(E("line", { x1:0, y1:baseY, x2:w, y2:baseY, stroke:CO.hair, "stroke-width":1.2 }));

      for(var k=1;k<=6;k++){ var xk = sx(k); if(xk > w - 8) continue;
        svg.appendChild(E("line", { x1:xk, y1:baseY, x2:xk, y2:baseY+6, stroke:CO.hair, "stroke-width":1.2, opacity:0.7 }));
        var tk = E("text", { x:xk, y:baseY+20, "text-anchor":"middle", "class":"ig-tick", "font-size":fs }); tk.textContent = k + "σ"; svg.appendChild(tk); }

      svg.appendChild(E("circle", { cx:sx(0), cy:baseY, r:3.4, fill:CO.paper }));
      var av = E("text", { x:sx(0), y:baseY+20, "text-anchor":"middle", "class":"ig-avg", "font-size":fs }); av.textContent = g.avg; svg.appendChild(av);

      var stemsY = [baseY - geo.ampl*0.30, baseY - geo.ampl*0.22, baseY - geo.ampl*0.14];
      for(var ci=0; ci<c.length; ci++){ var cx = sx(parseFloat(c[ci][1])); var ty = stemsY[ci] != null ? stemsY[ci] : baseY - geo.ampl*0.2;
        svg.appendChild(E("line", { x1:cx, y1:baseY, x2:cx, y2:ty+8, stroke:CO.paperSoft, "stroke-width":0.8, opacity:0.32 }));
        svg.appendChild(E("circle", { cx:cx, cy:baseY, r:3.2, fill:CO.paper, opacity:0.9 }));
        var nm = E("text", { x:cx, y:ty, "text-anchor":"middle", "class":"ig-cname", "font-size":fs }); nm.textContent = c[ci][0]; svg.appendChild(nm);
        var cs = E("text", { x:cx, y:ty+fs+3, "text-anchor":"middle", "class":"ig-csig", "font-size":fs-1 }); cs.textContent = c[ci][1]; svg.appendChild(cs); }

      var stx = sx(stepanSigma);
      svg.appendChild(E("line", { x1:stx, y1:stepanNameY+30, x2:stx, y2:baseY-6, stroke:CO.terra, "stroke-width":1.6 }));
      svg.appendChild(E("circle", { cx:stx, cy:baseY, r:14, fill:CO.terra, opacity:0.28, filter:"url(#igGlow)" }));
      svg.appendChild(E("circle", { cx:stx, cy:baseY, r:10.5, fill:CO.terra, filter:"url(#igGlow)" }));
      svg.appendChild(E("circle", { cx:stx, cy:baseY, r:5.5, fill:CO.clay }));
      var nmw = E("text", { x:stx, y:stepanNameY, "text-anchor":"middle", "class":"ig-name", "font-size":Math.max(20, fs*2.1) }); nmw.textContent = g.name; svg.appendChild(nmw);
      var nsg = E("text", { x:stx, y:stepanNameY+fs+8, "text-anchor":"middle", "class":"ig-namesig", "font-size":fs+2 }); nsg.textContent = g.sig; svg.appendChild(nsg);

      curGroup = E("g", { opacity:0 }); curGroup.style.transition = "opacity .18s ease";
      curLine = E("line", { x1:0, y1:0, x2:0, y2:baseY, stroke:CO.paper, "stroke-width":1, "stroke-dasharray":"2 4", opacity:0.5 });
      curDot = E("circle", { cx:0, cy:baseY, r:3, fill:CO.paper, opacity:0.8 });
      curSig = E("text", { x:0, y:0, "text-anchor":"middle", "class":"ig-readsig", "font-size":fs+1 });
      curGroup.appendChild(curLine); curGroup.appendChild(curDot); curGroup.appendChild(curSig);
      svg.appendChild(curGroup);
    }

    function pointerX(evt){ var r = svg.getBoundingClientRect(); var cx = (evt.touches ? evt.touches[0].clientX : evt.clientX) - r.left; return cx; }
    function moveCursor(x){ if(!geo || !curGroup) return;
      x = Math.max(geo.padL, Math.min(geo.w - 6, x));
      var s = sigAt(x); if(s < 0) s = 0;
      curLine.setAttribute("x1", x); curLine.setAttribute("x2", x);
      curDot.setAttribute("cx", x);
      curSig.setAttribute("x", x); curSig.setAttribute("y", geo.baseY - 16); curSig.textContent = s.toFixed(1) + "σ";
